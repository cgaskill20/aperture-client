...
describe('Util', function () {
    describe('getParamsAndTagsFromGeoJsonFeature()', function () {
        it('gets params and tags from the object passed to it', function () {
            assert.deepEqual(Util.getParamsAndTagsFromGeoJsonFeature({ properties: { tags: { man_made: "water_works", waterway: "dam" } } }), { params: ["man_made", "waterway"], tagsObj: { man_made: "water_works", waterway: "dam" } });
        });
    });
    describe('getLatLngFromGeoJsonFeature()', function () {
        it('gets latlng from the object passed to it', function () {
            assert.deepEqual(Util.getLatLngFromGeoJsonFeature({ geometry: { type: "none", coordinates: L.latLng(40, 90) } }), { lat: 0, lng: 0 });
            assert.deepEqual(Util.getLatLngFromGeoJsonFeature(pointGeoJSON), { lat: 40.567845225922326, lng: -105.03530293462781 });
            assert.deepEqual(Util.getLatLngFromGeoJsonFeature(lineGeoJSON), { lat: 40.78584, lng: -103.899475 });
            assert.deepEqual(Util.getLatLngFromGeoJsonFeature(polyGeoJSON), { lat: 40.40303938724696, lng: -105.13864988486216 });
        });
    });
    describe('getFeatureType()', function () {
        it('gets type from the geoJSON passed to it', function () {
            assert.deepEqual(Util.getFeatureType({ geometry: { type: "none", coordinates: L.latLng(40, 90) } }), -1);
            assert.deepEqual(Util.getFeatureType(pointGeoJSON), Util.FEATURETYPE.point);
            assert.deepEqual(Util.getFeatureType(lineGeoJSON), Util.FEATURETYPE.lineString);
            assert.deepEqual(Util.getFeatureType(polyGeoJSON), Util.FEATURETYPE.polygon);
        });
    });
    describe('simplifyGeoJSON()', function () {
        it('simplifies GeoJSON using the simplify.js library', function () {
            Util.simplifyGeoJSON(polyGeoJSON, 0.01);
    ...
    describe('fixGeoJSONID()', function () {
        it('if a geoJSON feature is misidentified as a line, make it a poly', function () {
            ...
            Util.fixGeoJSONID(polyGeoJSONClone);
            assert.deepEqual(polyGeoJSONClone.geometry.type, "Polygon");

            polyGeoJSONClone.geometry.type = "LineString";
            const newTest = Util.createGeoJsonObj([polyGeoJSONClone]);
            Util.fixGeoJSONID(newTest);
    ...
    describe('normalizeFeatureID()', function () {
        it('make weird mongodb ids into string .ids', function () {
            ...
            Util.normalizeFeatureID(polyGeoJSONClone);
    ...
    describe('removePropertiesPrefix()', function () {
        it('removes properties. from name of variable', function () {
            assert.deepEqual(Util.removePropertiesPrefix("properties.test"), "test");
        });
    });
    describe('leafletBoundsToGeoJSONPoly()', function () {
        it('removes properties. from name of variable', function () {
            assert.deepEqual(Util.leafletBoundsToGeoJSONPoly(JSON.parse('{"_southWest":{"lat":40.32874883206875,"lng":-105.53878784179689},"_northEast":{"lat":40.817446884558805,"lng":-104.63516235351562}}')), [
                [
                    -105.53878784179689,
                    40.32874883206875
                ],
    ...
    describe('mirrorLatLng()', () => {
        it('swaps entires when passed an array', () => {
            let latlng = [20, 30];
            assert(Util.mirrorLatLng(latlng)[0] === 30);
            assert(Util.mirrorLatLng(latlng)[1] === 20);
        });

        it('swaps entires when passed an object', () => {
            let latlng = { lat: 20, lng: 30, };
            assert(Util.mirrorLatLng(latlng).lat === 30);
            assert(Util.mirrorLatLng(latlng).lng === 20);
        });
    });
    describe('arePointsApproximatelyInBounds()', () => {
        it('can guess for small numbers of points', () => {
            ...
            assert(Util.arePointsApproximatelyInBounds(points, bounds));
            assert(!Util.arePointsApproximatelyInBounds(points, badbounds));
        });
    });
    ...
    describe('isInBounds()', () => {
        it('can determine bounds for multipolygon', () => {
            ...
            debugger;
            assert(Util.isInBounds(entry, L.latLngBounds([30, -100], [50, -120])));
            assert(Util.isInBounds(entry, L.latLngBounds([30, 100], [50, 120])));
            assert(!Util.isInBounds(entry, L.latLngBounds([30, 20], [50, 30])));
        });
    });
});
